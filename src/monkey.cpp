...
Malpa::Malpa(Texture2D monkeySprite)
{
    ...
    width = 50;
    height = 50;
    separationRange = 50;              // odległość jaką małpki starają się utrzymać pomiędzy sobą
    maxspeed =  0.5;                       // max prędkość danej małpki
    maxSeparationForce = 0.125;       // siła odpychania się między małpkami
...
{
  Vector2 vec{0,static_cast<float>(GetRandomValue(80,GetScreenHeight()))};
  return vec;
}
