...
bool Snake::checkCollisionWithEdges(Vector2 &pos,Sound BCS)
{
    ...
    bool collide = false;
    if (pos.x > width-4)  {{pos.x = width-4; PlaySound(BCS);} if(angle>PI) angle=angle-PI-2*angle; else angle=PI-angle;  collide=true;}
    if (pos.x < 3)        {{pos.x = 3; PlaySound(BCS);}        if(angle>PI) angle=2*PI-(angle-PI); else angle=PI-angle;  collide=true; }
    if (pos.y > height-6) {{pos.y = height-6;PlaySound(BCS); } if(angle>PI/2) angle=2*PI-angle; else angle=2*PI-angle;  collide=true; }
    if (pos.y < 64)        {{pos.y = 65; PlaySound(BCS);}       if(angle>1.5*PI) angle=angle-PI-(2*(angle-1.5*PI)); else angle=angle-PI+(2*(1.5*PI-angle));  collide=true;}
    return collide;
}
