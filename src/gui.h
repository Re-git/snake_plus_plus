    ...
    if(CheckCollisionPointRec(GetMousePosition(), btnBounds_back))
      {
        ...
        DrawText(TextFormat("YOUR SCORE: %d", points),70+GetScreenWidth()/2 -scorebox.width/2,170+GetScreenHeight()/2-scorebox.height/2, 40,BLACK);
        DrawText(TextFormat("TIME SURVIVED: %ds", frameCounter/60),30+(GetScreenWidth()/2)-scorebox.width/2,120+GetScreenHeight()/2-scorebox.height/2, 40,BLACK);
        DrawTexture(kursor,GetMouseX(),GetMouseY(),WHITE);
        EndDrawing();
}
...
void Gui::drawInGameGui(int points, int frameCounter)
{
    ...
    //GÓRNA CZĘŚĆ GUI
      ...
      DrawText(TextFormat("CZAS: %d",frameCounter/60),screenWidth-250,15,35,RAYWHITE);
}